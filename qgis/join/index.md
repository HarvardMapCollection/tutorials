...
Often, spreadsheets contain useful information we want to map. GIS software, however, does not know how to translate a spreadsheet without any *inherent spatial information* (such as point coordinates) into a visal map.
...
Despite the obvious role of geography in this data (the data is arranged by town), if we were to bring this spreadsheet into a GIS program, the program would not know, by itself, which column refers to the geography. The formats `.csv` or `.xlsx` are not `spatial data formats`. 

On the other hand, take this Massachusetts municipalities dataset, which is downloadable from the state government's open data portal as a `shapefile`, an inherently geospatial format.
...
In this tutorial, we will walk step-by-step through performing a join in QGIS, following along with the census data we obtained in the [Census Data to ArcGIS Online Starter Pack Series](https://harvardmapcollection.github.io/tutorials/census/census2agol/).
